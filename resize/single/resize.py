...
'''
   neural net to prediction letter. x -> category
   x -> category -> resized X aka x -> category + category -> X KNOWN
   x -encoder-> (logits -> softmax trick) -generator-> resized X
...
def simple_model():
  ...
  ol = keras.layers.Lamba(lambda: x * 1000)(ol)
  ol = keras.activations.softmax()(ol)
