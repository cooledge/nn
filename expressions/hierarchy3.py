...
class HierarchyModel:
  ...
  def init_hmodel(self):
    ...
    # the one hot vector of types 
    self.model_hierarchy_output = tf.clip_by_value(self.model_input + l1 + l2 + l3 + l4, 0.0, 1.0)
    # returns 1 if input is a single leaf node
    self.model_is_leaf_output = tf.nn.relu(2 - tf.reduce_sum(self.model_hierarchy_output)) 
  ...
  def infer(self, type):
    ...
    return session.run( [self.model_hierarchy_output, self.model_is_leaf_output], feed )
...
while True:
  ...
  output, is_leaf = hierarchy_model.infer(type)
