...
# the select op
model_outputs = tf.placeholder(tf.float32, shape=(1, number_of_ops), name="outputs")
...
model_loss = tf.losses.softmax_cross_entropy(model_outputs, model_reduced)
...
for epoch in range(epochs):
  for batch in range(len(inputs)):
    feed_dict = { model_inputs: inputs[batch:batch+1], model_outputs: labels[batch:batch+1] }
...
while True:
  ex_string = raw_input("Enter an sentence if you dare: ")
  if ex_string == "":
    break
